...
class SettingsView(ft.Column):
    def __init__(self, page: ft.Page, schedule, change_screen_func):
        ...
        self.numbers_switch = ft.Switch(
            label="הפעל מספור מפגשים אוטומטי" if self.schedule.language == "he" else "Enable auto meeting numbers",
        ...
        def pick_date(e, is_start):
            def handle_change(ev):
                ...
                if val:
                    # התיקון הקריטי: המרה בטוחה כדי שטקסט לא ייכנס למערכת
        ...
        date_section = ft.Column([
            ...
            ft.Row([
                ft.ElevatedButton(t("settings.change_start"), icon="calendar_month", on_click=lambda e: pick_date(e, True)),
                self.start_text
            ]),
            ft.Row([
                ft.ElevatedButton(t("settings.change_end"), icon="calendar_month", on_click=lambda e: pick_date(e, False)),
                self.end_text
            ])
        ], spacing=10)
